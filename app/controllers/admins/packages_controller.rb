module Admins
  class PackagesController < ApplicationController
    ...
    def create
      ...
      respond_to do |format|
        if @package.save
          format.html { redirect_to admins_packages_path, notice: 'Package was successfully created.' }
    ...
    def destroy
      ...
      respond_to do |format|
        format.html { redirect_to admins_packages_url, notice: 'Package was successfully deleted.' }
        format.json { head :no_content }
      end
    end
    ...
    def destroy_quiz_from_package
      begin
        ...
        redirect_to [:admins,@package], notice: 'Quiz was successfully removed from the package.'
      rescue Exception => ex
        logger.info("Error 1001: Error removing quiz from package : " + ex.message)
        redirect_to [:admins,@package], notice: 'Error 1001: Error removing quiz from the package. '
      end
    end
    ...
    def add_quiz_to_package
      if params[:quiz][:id] == "" || params[:quiz][:id] == nil
        ...
        if package_quiz.save
          redirect_to [:admins,@package], notice: 'Quiz added successfully to the package.'
        else
          redirect_to [:admins,@package], notice: 'Error adding quiz to the package.'
        end
      end
    end
