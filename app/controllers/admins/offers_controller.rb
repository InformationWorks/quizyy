  ...
  class OffersController < ApplicationController
    ...
    def add_quiz_to_offer
      if params[:quiz][:id] == "" || params[:quiz][:id] == nil
        ...
        if offer_quiz.save
          redirect_to [:admins,@offer], notice: 'Quiz added successfully to the offer.'
        else
          redirect_to [:admins,@offer], notice: 'Error adding quiz to the offer.'
        end
      end
    end
    ...
    def add_package_to_offer
      if params[:package][:id] == "" || params[:package][:id] == nil
        ...
        if offer_package.save
          redirect_to [:admins,@offer], notice: 'Package added successfully to the offer.'
        else
          redirect_to [:admins,@offer], notice: 'Error adding package to the offer.'
        end
      end
    end
    ...
    def destroy_quiz_from_offer
      begin
        ...
        redirect_to [:admins,@offer], notice: 'Quiz was successfully removed from the offer.'
      rescue Exception => ex
        logger.info("Error 1001: Error removing quiz from offer : " + ex.message)
        redirect_to [:admins,@offer], notice: 'Error 1001: Error removing quiz from the offer. '
      end
    end
    ...
    def destroy_package_from_offer
      begin
        ...
        redirect_to [:admins,@offer], notice: 'Package was successfully removed from the offer.'
      rescue Exception => ex
        logger.info("Error 1001: Error removing package from offer : " + ex.message)
        redirect_to [:admins,@offer], notice: 'Error 1001: Error removing package from the offer. '
      end
    end
    ...
    def add_emails_to_offer
      ...
      if params[:emails] == nil || params[:emails].strip == ""
        redirect_to [:admins,@offer], notice: 'Please enter atleast one email.'
      else
        ...
        begin
          ...
          redirect_to [:admins,@offer], notice: 'Emails added successfully to the offer.'
    ...
    def destroy_email_from_offer
      begin
        ...
        redirect_to [:admins,@offer], notice: 'Email was successfully removed from the offer.'
      rescue Exception => ex
        logger.info("Error 1001: Error removing email from offer : " + ex.message)
        redirect_to [:admins,@offer], notice: 'Error 1001: Error removing email from the offer. '
      end
    end
